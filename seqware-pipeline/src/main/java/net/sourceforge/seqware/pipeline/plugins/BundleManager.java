...
public class BundleManager extends Plugin {
	...
	public BundleManager() {
		...
		parser.acceptsAll(Arrays.asList("install-zip-only", "izo"),
		...
		parser.acceptsAll(Arrays.asList("install-dir-only", "ido"),
		...
		parser.acceptsAll(Arrays.asList("path-to-package", "p"),
		...
		parser.acceptsAll(Arrays.asList("workflow", "w"),
				...
				.withRequiredArg();
		parser.acceptsAll(Arrays.asList("version", "workflow-version"),
		...
		parser.acceptsAll(Arrays.asList("download-url"),
				"Downloads a workflow bundle zip from a URL to the local directory.").withRequiredArg();
		parser.acceptsAll(Arrays.asList("metadata", "m"), "Specify the path to the metadata.xml file.")
				.withRequiredArg();
		parser.acceptsAll(Arrays.asList("human-expanded", "he"),
				"Optional: will print output in expanded human friendly format");
		parser.acceptsAll(Arrays.asList("human-aligned", "ha"),
				"Optional: will print output in aligned human friendly format");
		this.outFile = parser
				.acceptsAll(Arrays.asList("out"), "Optional: Will output a list of workflows installed by sw_accession")
				.withRequiredArg();
	...
	public ReturnValue do_run() {
		...
		if (options.has("bundle") && (options.has("list") || options.has("help"))) {
			...
			for (WorkflowInfo wi : bi.getWorkflowInfo()) {
				...
				println("  Requirements Compute: " + wi.getComputeReq() + " Memory: " + wi.getMemReq() + " Network: "
						+ wi.getNetworkReq() + "\n");
