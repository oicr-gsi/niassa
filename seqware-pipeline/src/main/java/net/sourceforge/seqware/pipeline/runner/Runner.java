...
public class Runner {
	...
	public Runner() {
		...
		PARSER.accepts("metadata-config-database",
				...
				.describedAs("Example: jdbc:postgresql://127.0.0.1/seqware_meta_db");
		...
		PARSER.accepts("metadata-parentID",
		...
		PARSER.accepts("metadata-parentID-file",
		...
		PARSER.accepts("metadata-parent-accession",
				...
				.withRequiredArg().withValuesSeparatedBy(',').ofType(Integer.class).describedAs(
		...
		PARSER.accepts("metadata-processingID-file",
		...
		PARSER.accepts("metadata-workflow-accession",
		...
		PARSER.accepts("metadata-workflow-run-accession-output-file",
		...
		PARSER.accepts("metadata-workflow-run-ancestor-accession-input-file",
		...
		// Debugging stuff
		PARSER.accepts("sleep-between-steps",
				"Optional: For debugging, allows one to specify a time to sleep between steps").withRequiredArg()
	...
	private void writeStringToFile(File file, boolean append, String output) {
		...
		for (int i = 0; i <= maxTries; i++) {
			...
			if (maxTries == i) {
				...
				throw new ExitException(retval.getExitStatus());
			}
		}
	}
	...
	/**
	 * FIXME: this needs to be migrated to something that is ZIP64 aware. Try using
	 * the unzip feature of FileTools.java
	 *
	 * @param zipFile
	 *            a {@link java.lang.String} object.
	...
	private void checkSyntax() {
		...
		/**
		 * Why consider this wrong? Can a user module have their arugment as "--help"?
		 * If people want to see the Runner's help, they know that they should do it as
		 * optional argument.
		 */
		 ...
		 * "metadata-config-password", "metadata-parentID", "metadata-parentID-file",
		 * "metadata-processingID-file", "metadata-parent-accession",
		 * "metadata-parent-accession-file", "metadata-processing-accession-file",
		 * "metadata-tries-number", "metadata-tries-delay",
		 * "metadata-workflow-accession", "metadata-workflow-run-accession-output-file",
	...
	private void preProcessMetadata() {
		...
		if (optionHasOneOf(
				new String[] { "metadata-config-database", "metadata-config-username", "metadata-config-password" })) {
			// legacy mode
		...
		if (meta != null) {
			...
			if (parentAccessionsArray.length > 0) {
				...
				if (metaret.getExitStatus() == ReturnValue.SUCCESS) {
					...
					throw new ExitException(ReturnValue.SQLQUERYFAILED);
				}
			} else {
		...
		for (ModuleMethod m : ModuleMethod.values()) {
			...
			if ((m == outStart) && (app.getStdoutFile() != null)) {
				try {
					...
					printAndAppendtoStderr(
							"Error in redirecting stdout to '" + app.getStdoutFile().getAbsolutePath() + "'.");
			...
			if ((m == errStart) && (app.getStderrFile() != null)) {
				try {
					...
					printAndAppendtoStderr(
							"Error in redirecting stderr to '" + app.getStderrFile().getAbsolutePath() + "'.");
					printAndAppendtoStderr(e.getMessage());
					throw new ExitException(ReturnValue.INVALIDFILE);
				}
			}
