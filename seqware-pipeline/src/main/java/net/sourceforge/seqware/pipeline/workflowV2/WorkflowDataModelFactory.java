...
public class WorkflowDataModelFactory {
	...
	/**
	 * This constructs the factory with only the parameters which shouldn't change
	 ...
	 * @return
	 */
	public synchronized AbstractWorkflowDataModel getWorkflowDataModel(String bundlePath, int workflowAccession,
			int workflowRunAccession, String workflowEngine) {
		...
		// 0.13.6.5 : The Java workflow launcher was not originally designed to
		// schedule, hence it is not properly getting
		...
		// merge command line option with configs, command-line options should override
		// parent accession set above if present
		...
		// in 1.1 we're going to make metadata writeback of at least workflow runs
		// mandatory
		...
		if (workflowJava) {
			try {
				dataModel.setupDirectory();
				dataModel.setupFiles();
				dataModel.setupWorkflow();
				dataModel.setupEnvironment();
	 ...
	 * location.
	 * 
	 * It's used when the local workflow bundle dir is null or doesn't exist which
	 * is a sign that the workflow bundle should be retrieved from the permanent
	 * location
	 * 
	 * @param permLoc
	 * @return
	...
	/**
	 * This method is badly named now. There are no command-line options if we
	 * always schedule. Instead we only need to retain the process of getting
	 * information from the DB into the workflow data model so that we can use it.
	 * 
	 * @param model
	 * @param workflowAccession
	 * @param workflowRunAccession
	...
	private static ArrayList<String> parseParentAccessions(Map<String, String> map) {
		...
		for (String key : map.keySet()) {
			if (ReservedIniKeys.PARENT_ACCESSION.getKey().equals(key)
					|| ReservedIniKeys.PARENT_UNDERSCORE_ACCESSIONS.getKey().equals(key)
