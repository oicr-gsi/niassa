...
public class Main {
	...
	private static final SortedSet<String> ANNO_OBJS = new TreeSet<>(Arrays.asList("experiment", "file", "ius", "lane",
			"processing", "sample", "sequencer-run", "study", "workflow", "workflow-run"));
			...
			if (!ANNO_OBJS.contains(obj)) {
				...
				if (isHelp(args, true)) {
					...
					if (swid != null && key != null && val != null && skip == false && csv == null) {
						...
						run("--plugin", "net.sourceforge.seqware.pipeline.plugins.AttributeAnnotator", "--", idFlag,
								swid, "--key", key, "--value", val);
						...
						if (reason == null) {
							...
							run("--plugin", "net.sourceforge.seqware.pipeline.plugins.AttributeAnnotator", "--", idFlag,
									swid, "--skip", "true", "--value", reason);
						}
					} else if (swid == null && key == null && val == null && skip == false && csv != null) {
						run("--plugin", "net.sourceforge.seqware.pipeline.plugins.AttributeAnnotator", "--", "--file",
								csv);
					} else {
	...
	private static void bundleInstall(List<String> args) {
		if (isHelp(args, true)) {
			...
			run("--plugin", "net.sourceforge.seqware.pipeline.plugins.BundleManager", "--", "--install", "--bundle",
					zip);
		}
	}
	...
	private static void bundleInstallDirOnly(List<String> args) {
		if (isHelp(args, true)) {
			...
			run("--plugin", "net.sourceforge.seqware.pipeline.plugins.BundleManager", "--", "--install-dir-only",
					"--bundle", zip);
		}
	}
	...
	private static WorkflowInfo findWorkflowInfo(File dir, String name, String version) {
		...
		if (found.isEmpty()) {
			if (name == null && version == null) {
				...
				kill("seqware: no workflow with version '" + version + "' defined in "
			...
			if (name == null && version == null) {
				...
				kill("seqware: multiple workflows with name '" + name + "' defined in "
						+ bi.parsedFrom().getAbsolutePath());
			} else if (name == null) {
	...
	private static void bundleLaunch(List<String> args) {
		if (isHelp(args, true)) {
			...
			String[] runParams;
			if (engine == null) {
				runParams = new String[] { "--plugin", "io.seqware.pipeline.plugins.WorkflowLifecycle", "--", "--wait",
						"--bundle", dir, "--workflow", name, "--version", version, "--ini-files", cdl(inis) };
			} else {
				runParams = new String[] { "--plugin", "io.seqware.pipeline.plugins.WorkflowLifecycle", "--", "--wait",
						"--bundle", dir, "--workflow", name, "--version", version, "--ini-files", cdl(inis),
	...
	private static void bundleDryRun(List<String> args) {
		if (isHelp(args, true)) {
			...
			if (engine == null) {
				run("--plugin", "io.seqware.pipeline.plugins.WorkflowLifecycle", "--", "--bundle", dir, "--workflow",
						name, "--version", version, "--ini-files", cdl(inis), "--no-run");
			} else {
				run("--plugin", "io.seqware.pipeline.plugins.WorkflowLifecycle", "--", "--bundle", dir, "--workflow",
						name, "--version", version, "--ini-files", cdl(inis), "--workflow-engine", engine, "--no-run");
			}
		}
	}
		...
		if (isHelp(args, true)) {
			...
			if (args.size() == 2) {
				...
				run("--plugin", "net.sourceforge.seqware.pipeline.plugins.ModuleRunner", "--", "--module",
						"net.sourceforge.seqware.pipeline.modules.utilities.ProvisionFiles", "--no-metadata", "--",
						"--force-copy", "--input-file", src, destFlag, dest);
			} else {
	...
	private static void runCreateTable(List<String> args, String table, String... cols) {
		if (flag(args, "--interactive")) {
			...
			// workflow-run tables can have (potentially multiple) file parameters, pass
			// these through after doing some validation
			if (table.equals("workflow_run")) {
				...
				// workflow runs should also have parent accessions in order to be visible to
				// deciders
	 ...
	 *
	 * @param args
	 *            A list of command line args to be parsed
	 *
	 * @return IUS SWID
	 */
	public static Integer createIusLimsKey(List<String> args) {
		...
		if (options.has(helpOpt)) {
			...
			ZonedDateTime lastModified = ZonedDateTime.parse(options.valueOf(lastModifiedOpt));
			...
			out("Created LimsKey[%s,%s,%s,%s] with SWID: %s", provider, id, version, lastModified.toString(),
					limsKeySwid.toString());
	...
	private static void createLane(List<String> args) {
		if (isHelp(args, true)) {
			...
			runCreateTable(args, "lane", "cycle_descriptor", "description", "lane_number",
					"library_selection_accession", "library_source_accession", "library_strategy_accession", "name",
					"sequencer_run_accession", "skip", "study_type_accession");
		}
	}
	...
	private static void createSequencerRun(List<String> args) {
		if (isHelp(args, true)) {
			...
			runCreateTable(args, "sequencer_run", "description", "file_path", "name", "paired_end",
					"platform_accession", "skip");
		}
	}
	...
	private static void createStudy(List<String> args) {
		if (isHelp(args, true)) {
			...
			runCreateTable(args, "study", "accession", "center_name", "center_project_name", "description",
					"study_type", "title");
		}
	}
		...
		if (isHelp(args, true)) {
			...
			if (null != obj)
				...
				case "workflow-run":
					...
					break;
				default:
					kill("seqware: 'create %s' is not a seqware command. See 'seqware create --help'.", obj);
					break;
				}
		}
	}
	...
	private static void workflowIni(List<String> args) {
		if (isHelp(args, true)) {
			...
			try {
				...
				System.setOut(temp);
				run("--plugin", "net.sourceforge.seqware.pipeline.plugins.BundleManager", "--",
						"--list-workflow-params", "--workflow-accession", id);
	 ...
	 * where output contains unintended formatting strings, which would break the
	 * {@link #out(String format, Object... args)} function. For example, if you try
	 * to print an INI file containing the line "refExclude=XX,GL%,hs37d5,XX_001234"
	 * the <i>substring</i> "%,h" will cause String.format to throw an exception and
	 * fail. So it is sometimes necessary to print output with no consideration to
	 * formatting.
	 *
	 * @param output
	 */
	...
	public static void main(String[] argv) {
		try {
			mainInternal(argv);
		} catch (ExitException k) {
			System.exit(k.status);
		}

	}

	public static void mainInternal(String[] argv) {
