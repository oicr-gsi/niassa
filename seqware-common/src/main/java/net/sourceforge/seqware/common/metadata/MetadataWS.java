...
public class MetadataWS implements Metadata {
	...
	static {
		...
		// This is here to get Maven to not mangle the dependencies
		Class<?> x = Versioned.class;
	}
	...
	public ReturnValue addWorkflow(String name, String version, String description, String baseCommand,
			...
			boolean storeArchiveZip, String workflowClass, String workflowType, String workflowEngine,
			String seqwareVersion) {

		ReturnValue ret = new ReturnValue(ReturnValue.SUCCESS);

		Workflow workflow = convertParamsToWorkflow(baseCommand, name, description, version, configFile,
		...
		try {
			...
			System.out.println("MetadataWS.addWorkflow: Added '" + workflow.getName() + "' (SWID: " + workflow.getSwAccession() + ")");
	...
	public static Workflow convertParamsToWorkflow(String baseCommand, String name, String description, String version1,
			String configFile, boolean storeProvisionDir, String provisionDir, String templateFile,
			boolean storeArchiveZip, String archiveZip, String workflow_class, String workflow_type,
			String workflow_engine, String seqware_version) {
		Workflow workflow;
	 ...
	 */
	@Override
	public ReturnValue addStudy(String title, String description, String centerName, String centerProjectName,
			Integer studyTypeId) {
	 ...
	 */
	@Override
	public ReturnValue addSample(Integer experimentAccession, Integer parentSampleAccession, Integer organismId,
	 ...
	 */
	@Override
	public ReturnValue addSequencerRun(Integer platformAccession, String name, String description, boolean pairdEnd,
			boolean skip, String filePath, SequencerRunStatus status) {
		ReturnValue ret = new ReturnValue(ReturnValue.SUCCESS);
	...
	@Override
	public ReturnValue addIUS(Integer laneAccession, Integer sampleAccession, String name, String description,
			String barcode, boolean skip) {
		ReturnValue ret = new ReturnValue(ReturnValue.SUCCESS);
	...
	public List<ParentAccessionModel> getViaParentAccessions(int[] potentialParentAccessions) {
		...
		for (int parentAccession : potentialParentAccessions) {
			ParentAccessionModel resolveParentAccession = this
					.resolveParentAccession("/" + String.valueOf(parentAccession));
	 ...
	 */
	private void addParentsAndChildren(Processing p, String[] parentIds, String[] childIds)
			throws ResourceException, IOException, JAXBException {
	 ...
	 */
	@Override
	public ReturnValue associate_processing_event_with_parents_and_child(int processingID, int[] parentIDs,
			int[] childIDs) {
		ReturnValue ret = new ReturnValue();
		try {
	 ...
	 */
	@Override
	public ReturnValue update_workflow_run(int workflowRunId, String pegasusCmd, String workflowTemplate,
			WorkflowRunStatus status, String statusCmd, String workingDirectory, String dax, String ini, String host,
		...
		try {
			...
			accession = wr.getSwAccession();
			convertParamsToWorkflowRun(wr, pegasusCmd, workflowTemplate, status, statusCmd, workingDirectory, dax, ini,
	...
	protected static void convertParamsToWorkflowRun(WorkflowRun wr, String pegasusCmd, String workflowTemplate,
			WorkflowRunStatus status, String statusCmd, String workingDirectory, String dax, String ini, String host,
			String stdErr, String stdOut, String workflowEngine, Set<Integer> inputFiles) {
	 ...
	 * {@inheritDoc}
	 *
	 * FIXME: this is a hack, will need to add an object layer between this metadata
	 ...
	 */
	@Override
	public String getWorkflowRunReport(Integer workflowSWID, WorkflowRunStatus status, Date earliestDate,
			Date latestDate) {
	...
	public List<Lane> getLanesFrom(int sequencerRunAccession) {
		try {
			...
			LaneList list = ll.existsObject("/sequencerruns/" + sequencerRunAccession + "/lanes", "", jaxb,
					new LaneList(), LaneList.class);
			if (list != null) {
				return list.getList();
			}
		} catch (JAXBException ex) {
	...
	public List<IUS> getIUSFrom(int laneOrSampleAccession) {
		...
		try {
			...
			IUSList list = (IUSList) ll.existsObject(sb.toString(), "", jaxb, new IUSList(), IUSList.class);
			if (list != null) {
				return list.getList();
			}
		} catch (JAXBException ex) {
	...
	public List<Experiment> getExperimentsFrom(int studyAccession) {
		try {
			...
			ExperimentList list = (ExperimentList) ll.existsObject("/studies/" + studyAccession + "/experiments", "",
					jaxb, new ExperimentList(), ExperimentList.class);
			if (list != null) {
				return list.getList();
			}
		} catch (JAXBException ex) {
	...
	public List<Sample> getSamplesFrom(int experimentAccession) {
		try {
			...
			SampleList list = (SampleList) ll.existsObject("/experiments/" + experimentAccession + "/samples", "", jaxb,
	...
	public List<Sample> getChildSamplesFrom(int parentSampleAccession) {
		try {
			JaxbObject<SampleList> jaxb = new JaxbObject<>();
			SampleList list = (SampleList) ll.existsObject("/samples/" + parentSampleAccession + "/children", "", jaxb,
					new SampleList(), SampleList.class);
			if (list != null) {
				return list.getList();
			}
	...
	public List<Sample> getParentSamplesFrom(int childSampleAccession) {
		try {
			JaxbObject<SampleList> jaxb = new JaxbObject<>();
			SampleList list = (SampleList) ll.existsObject("/samples/" + childSampleAccession + "/parents", "", jaxb,
					new SampleList(), SampleList.class);
			if (list != null) {
				return list.getList();
			}
	...
	public List<SampleProvenanceDto> getSampleProvenance() {
		try {
			...
			SampleProvenanceDtoList list = ll.findObject("/reports/sample-provenance", "", jaxb,
	...
	public List<LaneProvenanceDto> getLaneProvenance() {
		try {
			...
			LaneProvenanceDtoList list = ll.findObject("/reports/lane-provenance", "", jaxb,
	...
	protected class LowLevel {
		...
		public <T> T existsObject(String uri, String searchString, JaxbObject<T> jaxbObject, T parent, Class<T> type)
				throws JAXBException {
			T object = parent;
			try {
		...
		private List<Study> findStudies() throws IOException, JAXBException {
			...
			StudyList list = (StudyList) findObject("/studies", "", jaxb, new StudyList(), StudyList.class);
			if (list != null) {
				return list.getList();
			}
			return null;
		...
		private List<ExperimentLibraryDesign> findExperimentLibraryDesigns() throws IOException, JAXBException {
			...
			ExperimentLibraryDesignList list = (ExperimentLibraryDesignList) findObject("/experimentlibrarydesigns", "",
					jaxb, new ExperimentLibraryDesignList(), ExperimentLibraryDesignList.class);
			if (list != null) {
				return list.getList();
			}
			return null;
		...
		private List<ExperimentSpotDesignReadSpec> findExperimentSpotDesignReadSpecs()
				throws IOException, JAXBException {
			JaxbObject<ExperimentSpotDesignReadSpecList> jaxb = new JaxbObject<>();
			ExperimentSpotDesignReadSpecList list = (ExperimentSpotDesignReadSpecList) findObject(
					"/experimentspotdesignreadspecs", "", jaxb, new ExperimentSpotDesignReadSpecList(),
		...
		private List<ExperimentSpotDesign> findExperimentSpotDesigns() throws IOException, JAXBException {
			...
			ExperimentSpotDesignList list = (ExperimentSpotDesignList) findObject("/experimentspotdesigns", "", jaxb,
					new ExperimentSpotDesignList(), ExperimentSpotDesignList.class);
			if (list != null) {
				return list.getList();
			}
			return null;
		...
		private List<LibraryStrategy> findLibraryStrategies() throws IOException, JAXBException {
			...
			LibraryStrategyList list = (LibraryStrategyList) findObject("/librarystrategies", "", jaxb,
					new LibraryStrategyList(), LibraryStrategyList.class);
			if (list != null) {
				return list.getList();
			}
			return null;
		...
		private List<LibrarySelection> findLibrarySelections() throws IOException, JAXBException {
			...
			LibrarySelectionList list = (LibrarySelectionList) findObject("/libraryselections", "", jaxb,
					new LibrarySelectionList(), LibrarySelectionList.class);
			if (list != null) {
				return list.getList();
			}
			return null;
		...
		private List<LibrarySource> findLibrarySources() throws IOException, JAXBException {
			...
			LibrarySourceList list = (LibrarySourceList) findObject("/librarysources", "", jaxb,
					new LibrarySourceList(), LibrarySourceList.class);
			if (list != null) {
				return list.getList();
			}
			return null;
		...
		private Lane findLane(String searchString) throws IOException, JAXBException {
			...
			return (Lane) findObject("/lanes", searchString, jaxb, lane, Lane.class);
		}

		private IUS findIUS(String searchString) throws IOException, JAXBException {
			IUS ius = new IUS();
			JaxbObject<IUS> jaxb = new JaxbObject<>();
			return (IUS) findObject("/ius", searchString, jaxb, ius, IUS.class);
		}

		private LimsKey findLimsKey(String searchString) throws IOException, JAXBException {
			LimsKey limsKey = new LimsKey();
			JaxbObject<LimsKey> jaxb = new JaxbObject<>();
			return (LimsKey) findObject("/limskey", searchString, jaxb, limsKey, LimsKey.class);
		}
		...
		private WorkflowRun findWorkflowRun(String searchString) throws IOException, JAXBException {
			...
			return (WorkflowRun) findObject("/workflowruns", searchString, jaxb, wr, WorkflowRun.class);
		}
		...
		private List<WorkflowRun> findWorkflowRunsByFiles(List<Integer> files, List<Integer> workflows)
				throws IOException, JAXBException {
			JaxbObject<IntegerList> jaxb = new JaxbObject<>();
			...
			WorkflowRunList2 wrl2 = (WorkflowRunList2) this.addObject("/reports/fileworkflowruns/limit",
					workflows.size() > 0 ? "?workflows=" + workflowList : "", jaxb, fileInput, IntegerList.class,
					new JaxbObject<>(), new WorkflowRunList2(), WorkflowRunList2.class);
			return wrl2.getList();
		}

		private List<WorkflowRun> findWorkflowRunsByFiles(List<Integer> files, String search_type)
				throws IOException, JAXBException {
			...
			WorkflowRunList2 wrl2 = (WorkflowRunList2) findObject("/reports/fileworkflowruns",
					"?files=" + fileList + "&search=" + search_type, jaxb, w, WorkflowRunList2.class);
			return wrl2.getList();
		}
		...
		private Workflow findWorkflow(String searchString) throws IOException, JAXBException {
			...
			JaxbObject<Workflow> jaxb = new JaxbObject<>();
			return (Workflow) findObject("/workflows", searchString, jaxb, wr, Workflow.class);
		}
		...
		private SequencerRun findSequencerRun(String searchString) throws IOException, JAXBException {
			...
			return (SequencerRun) findObject("/sequencerruns", searchString, jaxb, study, SequencerRun.class);
		}

		private Study findStudy(String searchString) throws IOException, JAXBException {
			Study study = new Study();
			JaxbObject<Study> jaxb = new JaxbObject<>();
			return (Study) findObject("/studies", searchString, jaxb, study, Study.class);
		}
		...
		private Sample findSample(String searchString) throws IOException, JAXBException {
			...
			return (Sample) findObject("/samples", searchString, jaxb, study, Sample.class);
		}
		...
		private Experiment findExperiment(String searchString) throws IOException, JAXBException {
			...
			return (Experiment) findObject("/experiments", searchString, jaxb, exp, Experiment.class);
		}
		...
		private void get(String uri, Map<?, ? extends List<?>> params) throws IOException {
			...
			try {
				cr = resource.getChild(version + uri);
		...
		private List<Sample> matchSampleName(String name) throws IOException, JAXBException {
			JaxbObject<SampleList> jaxb = new JaxbObject<>();
			SampleList list = findObject("/samples", "?matches=" + name, jaxb, new SampleList(), SampleList.class);
			if (list != null) {
				return list.getList();
			}
			return null;
		...
		private <T> T getObject(String uri, String searchString, JaxbObject<T> jaxb, Class<T> type)
		...
		private void updateWorkflow(String searchString, Workflow parent)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<Workflow> jaxbProcess = new JaxbObject<>();
			updateObject("/workflows", searchString, jaxbProcess, parent, Workflow.class);
		}

		private void updateProcessing(String searchString, Processing parent)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<Processing> jaxbProcess = new JaxbObject<>();
			updateObject("/processes", searchString, jaxbProcess, parent, Processing.class);
		}

		private void updateWorkflowRun(String searchString, WorkflowRun parent)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<WorkflowRun> jaxb = new JaxbObject<>();
			updateObject("/workflowruns", searchString, jaxb, parent, WorkflowRun.class);
		}

		private void updateLane(String searchString, Lane parent) throws IOException, JAXBException, ResourceException {
			JaxbObject<Lane> jaxb = new JaxbObject<>();
			updateObject("/lanes", searchString, jaxb, parent, Lane.class);
		}

		private void updateIUS(String searchString, IUS parent) throws IOException, JAXBException, ResourceException {
			JaxbObject<IUS> jaxb = new JaxbObject<>();
			updateObject("/ius", searchString, jaxb, parent, IUS.class);
		}

		private void updateLimsKey(String searchString, LimsKey limsKey)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<LimsKey> jaxb = new JaxbObject<>();
			updateObject("/limskey", searchString, jaxb, limsKey, LimsKey.class);
		}

		private void updateSequencerRun(String searchString, SequencerRun parent)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<SequencerRun> jaxb = new JaxbObject<>();
			updateObject("/sequencerruns", searchString, jaxb, parent, SequencerRun.class);
		}

		private void updateSample(String searchString, Sample parent)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<Sample> jaxb = new JaxbObject<>();
			updateObject("/samples", searchString, jaxb, parent, Sample.class);
		}

		private void updateStudy(String searchString, Study parent)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<Study> jaxb = new JaxbObject<>();
			updateObject("/studies", searchString, jaxb, parent, Study.class);
		}

		private void updateFile(String searchString, File parent) throws IOException, JAXBException, ResourceException {
			JaxbObject<File> jaxb = new JaxbObject<>();
			updateObject("/files", searchString, jaxb, parent, File.class);
		}

		private void updateExperiment(String searchString, Experiment parent)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<Experiment> jaxb = new JaxbObject<>();
			updateObject("/experiments", searchString, jaxb, parent, Experiment.class);
		}
		...
		private Processing addProcessing(Processing processing) throws IOException, JAXBException, ResourceException {
			...
			return (Processing) addObject("/processes", "", jaxb, processing, Processing.class);
		}

		private File addFile(File workflowRun) throws IOException, JAXBException, ResourceException {
			JaxbObject<File> jaxb = new JaxbObject<>();
			return (File) addObject("/files", "", jaxb, workflowRun, File.class);
		}

		private WorkflowRun addWorkflowRun(WorkflowRun workflowRun)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<WorkflowRun> jaxb = new JaxbObject<>();
			return (WorkflowRun) addObject("/workflowruns", "", jaxb, workflowRun, WorkflowRun.class);
		}
		...
		private SequencerRun addSequencerRun(SequencerRun sequencerRun)
				throws IOException, JAXBException, ResourceException {
			JaxbObject<SequencerRun> jaxb = new JaxbObject<>();
			return (SequencerRun) addObject("/sequencerruns", "", jaxb, sequencerRun, SequencerRun.class);
		}

		private Lane addLane(Lane lane) throws IOException, JAXBException, ResourceException {
			JaxbObject<Lane> jaxb = new JaxbObject<>();
			return (Lane) addObject("/lanes", "", jaxb, lane, Lane.class);
		}

		private IUS addIUS(IUS ius) throws IOException, JAXBException, ResourceException {
			JaxbObject<IUS> jaxb = new JaxbObject<>();
			return (IUS) addObject("/ius", "", jaxb, ius, IUS.class);
		}

		private LimsKey addLimsKey(LimsKey limsKey) throws IOException, JAXBException, ResourceException {
			JaxbObject<LimsKey> jaxb = new JaxbObject<>();
			return (LimsKey) addObject("/limskey", "", jaxb, limsKey, LimsKey.class);
		}

		private <T> T addObject(String uri, String searchString, JaxbObject<T> jaxb, T parent, Class<T> clazz)
				throws IOException, JAXBException {
			return this.addObject(uri, searchString, jaxb, parent, clazz, jaxb, parent, clazz);
		}

		private <T, R> R addObject(String uri, String searchString, JaxbObject<T> jaxb, T parent, Class<T> clazz,
				JaxbObject<R> outJaxb, R outParent, Class<R> outClazz)
				throws IOException, JAXBException, ResourceException {
			Representation result = null;
			ClientResource cResource = resource.getChild(version + uri + searchString);
		...
		private void deleteObject(String uri, String searchString)
				throws IOException, JAXBException, ResourceException {
			Representation result = null;
			ClientResource cResource = null;
